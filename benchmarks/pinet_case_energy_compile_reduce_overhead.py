...
def main() -> None:
    ...
    model.compile_energy(backend="inductor", mode="reduce-overhead")
    model.eval()
    row = bench("energy_compile_reduce_overhead_forward_eval", lambda: model(clone_batch(batch), compute_forces=False), warmup=args.warmup, iters=args.iters)
