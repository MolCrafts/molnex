...
from pinet_gpu_diagnostics import bench, clone_batch, energy_loss, setup_case, write_case_result
...
def main() -> None:
    ...
    def step() -> None:
        ...
        out = model(clone_batch(batch), compute_forces=False)
        energy_loss(out, batch).backward()
        opt.step()

    row = bench("energy_compile_default_train_step", step, warmup=args.warmup, iters=args.iters)
