...
def test_neighbor_values(device, dtype, num_atoms, cutoff, all_pairs):
    ...
    # Compute results
    neighbors, deltas, distances, _ = get_neighbor_pairs(positions, cutoff=cutoff, max_num_pairs=max_num_pairs)
...
def test_neighbor_grads(device, dtype, num_atoms, grad):
    ...
    print(positions)
    neighbors, deltas, distances, _ = get_neighbor_pairs(positions, cutoff=cutoff)
...
def test_too_many_neighbors(device, dtype):
    ...
    with pytest.raises(RuntimeError):
        ...
        get_neighbor_pairs(positions, cutoff=1, max_num_pairs=1, check_errors=True)

    # checkErrors = False will never throw due to exceeding neighbours. In addition, the call will be compatible with CUDA graphs
    neighbors, deltas, distances, number_found_pairs = get_neighbor_pairs(positions, cutoff=1, max_num_pairs=1, check_errors=False)
...
def test_max_pairs_means_total(device, dtype):
    ...
    with pytest.raises(RuntimeError):
        ...
        get_neighbor_pairs(positions, cutoff=1, max_num_pairs=5, check_errors=True)
    get_neighbor_pairs(positions, cutoff=1, max_num_pairs=6, check_errors=True)
...
def test_is_cuda_graph_compatible():
    ...
    with pt.cuda.stream(s):
        for _ in range(3):
            neighbors, deltas, distances, _ = get_neighbor_pairs(positions, cutoff=cutoff, max_num_pairs=num_neighbors+1)
    pt.cuda.synchronize()
...
def test_periodic_neighbors(device, dtype):
    ...
    # Compute results
    neighbors, deltas, distances, _ = get_neighbor_pairs(positions, cutoff=cutoff, max_num_pairs=max_num_pairs, box_vectors=box_vectors)
...
def test_jit_script_compatible(device, dtype):
    ...
    class ForceModule(pt.nn.Module):
        ...
        def forward(self, positions):
            ...
            neighbors, deltas, distances, _ = get_neighbor_pairs(positions, cutoff=1.0)
