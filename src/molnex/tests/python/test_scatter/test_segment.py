...
def test_forward(test, reduce, dtype, device):
    ...
    fn = getattr(molpot_op.scatter, 'segment_' + reduce + '_csr')
    ...
    fn = getattr(molpot_op.scatter, 'segment_' + reduce + '_coo')
    jit = torch.jit.script(fn)
    out1 = fn(src, index)
    out2 = jit(src, index)
    if isinstance(out1, tuple):
...
def test_backward(test, reduce, device):
    ...
    assert gradcheck(molpot_op.scatter.segment_csr, (src, indptr, None, reduce))
    assert gradcheck(molpot_op.scatter.segment_coo,
...
def test_out(test, reduce, dtype, device):
    ...
    getattr(molpot_op.scatter, 'segment_' + reduce + '_csr')(src, indptr, out)
    assert torch.all(out == expected)

    out.fill_(-2)

    getattr(molpot_op.scatter, 'segment_' + reduce + '_coo')(src, index, out)
...
def test_non_contiguous(test, reduce, dtype, device):
    ...
    out = getattr(molpot_op.scatter, 'segment_' + reduce + '_csr')(src, indptr)
    ...
    out = getattr(molpot_op.scatter, 'segment_' + reduce + '_coo')(src, index)
    if isinstance(out, tuple):
        out, arg_out = out
        arg_expected = tensor(test['arg_' + reduce], torch.long, device)
        assert torch.all(arg_out == arg_expected)
