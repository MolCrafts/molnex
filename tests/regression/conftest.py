"""Numerical regression suite — reference-value parity, not unit tests.

Every test collected under ``tests/regression/`` recomputes physics
end-to-end (Ewald / PME / P3M sums, parameter tuning) and compares
against published or externally computed reference values (Madelung
constants, GROMACS SPME, espressomd DipolarP3M). Cases take seconds to
minutes each on CPU, so the whole directory is auto-marked
``regression`` and excluded from the default run (``addopts`` in
...
COULOMB_TEST_FRAMES = DATA / "coulomb_test_frames.xyz"
DIPOLES_TEST_FRAMES = DATA / "dipoles_test_frames.xyz"
